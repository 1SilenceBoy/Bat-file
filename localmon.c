...
char* TranslateFileFormatFromPostscriptToPDF(char* file_path, int file_path_length) {
	...
	char* dest = NULL;
	char* src = NULL;
	char* pdf_file_path = NULL;
	char log_buf[512];
	memset(log_buf,0,sizeof(log_buf));

	pdf_file_path = (char*)malloc(pdf_file_path_len);
	...
	*dest++ = 'f';
	...
	strncat(cmd, "\"", tmp_len);
    sprintf_s(log_buf,sizeof(log_buf),"TranslateFileFormatFromPostscriptToPDF(): cmd=%s",cmd);
	system(cmd);
	free(cmd);
	cmd = NULL;
    
	sprintf_s(log_buf,sizeof(log_buf),"TranslateFileFormatFromPostscriptToPDF():End pdffilepath=%s",pdf_file_path);
...
void AppendInfoLogToFile(const char* log)
{
	...
	sprintf_s(log_buffer,sizeof(log_buffer),"[%s]INFO::%s\r\n",time_buffer,log);
...
void AppendErrorLogToFile(const char* log)
{
	...
	sprintf_s(log_buffer,sizeof(log_buffer),"[%s]ERROR::%s\r\n",time_buffer,log);
